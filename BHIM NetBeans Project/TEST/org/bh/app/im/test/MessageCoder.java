...
public class MessageCoder {
    public static void main(String[] args) {
        ...
        System.out.println();
        start = System.nanoTime();
        BHIMMessage sendMessage = new BHIMMessage(rpMessage, // text
                                                  MessageType.PLAIN_TEXT, // type
                                                  0x1234_5678_CAFE_C0B0L, // sender
                                                  0xBAD_B0B, // recipient
                                                  0x0123457989ABCDEFL);   // time
        end = System.nanoTime();
        logTimespan("Creating IM message", end - start);

        System.out.println();
        start = System.nanoTime();
        byte[] toSend = sendMessage.getBytesToSend();
        end = System.nanoTime();
...
//        logTimespan("Printing serialized message", end - start);
        ...
        System.out.println();
        start = System.nanoTime();
        BHIMMessage receiveMessage = BHIMMessage.decode(toSend);
        end = System.nanoTime();
        logTimespan("Deserializing message", end - start);

        System.out.println("\tDeserializing was " + (receiveMessage.isValid() ? "" : "NOT ") + "successful!");
...
//        logTimespan("Stringifying message", end - start);
    ...
    private static void printBytes(byte[] bytes) {
        final int COLS = Integer.SIZE / Byte.SIZE;
    ...
    private static void logAllTimespans() {
        System.out.println("\r\n\r\n=== TOTALS ===\r\n\r\n");
